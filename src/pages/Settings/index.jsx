...
export default function Settings() {
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data.tasks) localStorage.setItem('productivity_tasks', JSON.stringify(data.tasks))
        if (data.habits) localStorage.setItem('productivity_habits', JSON.stringify(data.habits))
        if (data.notes) localStorage.setItem('productivity_notes', JSON.stringify(data.notes))
        if (data.sessions) localStorage.setItem('productivity_sessions', JSON.stringify(data.sessions))
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {/* Focus durations */}
      <div className="bg-card rounded-xl border border-border shadow-soft p-4">
        <h3 className="text-sm font-semibold mb-3">Focus Timer</h3>
